...
/////////////////////////////////////////////////////////////////////
int rc_analyser_cint(const char *outfile, const std::vector<const char *> infiles)
{
  // Load input files
  InputData_t idata;
  idata.tree = new TChain("tr");
  for (std::vector<const char *>::const_iterator it = infiles.begin(); it != infiles.end(); ++it) {
    std::cout << "Adding file: " << *it << std::endl;
    idata.tree->Add(*it);
  }
  ...
  odata.file = TFile::Open(outfile, "create");
  // If the file hasn't opened correctly, ofile has a null pointer (NULL)
  if(NULL == odata.file){ 
    std::cerr << "The output file cannot be opened newly with " << outfile << std::endl
